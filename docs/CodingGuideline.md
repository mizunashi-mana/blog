...
コードから明確に分かることは、コメントに記載しない。コメントは「なぜ」を説明し、「何をしているか」は書かない。

```typescript
// ❌ 間違い - コードから明確に分かることをコメントに記載
const userId = 123; // ユーザーIDを設定
users.push(user); // ユーザーを配列に追加

// ✅ 正しい - なぜその処理をするのかを説明
const userId = 123; // API制限により一時的にハードコードされた値
users.push(user); // レンダリング順序を保つため、先頭に追加
```
